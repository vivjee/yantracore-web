...
- [ ] **Phase 3 — Technologies + Activity into the group** (`/entryport` → `/activity` with a redirect); perfect the exit→enter cross-morph (frozen-router `AnimatePresence`). *Note: the Technologies `StarSystem` content was enhanced separately (frozen centre star, 9-planet stack + WordPress/Photoshop) but the route is not yet folded into `(orbital)`.*
...
- [ ] **Phase 6 — nav/IA wiring + Contact refresh + docs sweep.** Link `/book` · `/about` · `/work` into the TV chrome bar + Home nodes (the Home "About" node is still `soon`; the Book CTA still points to `/contact`); add the `/entryport` → `/activity` redirect once Activity folds in. (Staffing deferred.)
