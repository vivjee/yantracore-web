...
- **Channel pages** (`/about`, `/work`, `/activity`, `/contact`, `/book`, `/music`, console) — *change the channel*. They keep the CRT channel-change glitch. `/activity` (the globe) is a channel because its centerpiece isn't the logo.
