...
- **Mobile performance ("lite") mode** — gated on **`@media (pointer: coarse)`** (touch = phones any-orientation + tablets; excludes desktops/touch-laptops). Mobile GPUs can't composite the stacked animated blurs + `background-clip:text` filters + WebGL galaxy every frame, so dropped frames read as flicker. On touch we trade neon/animation for a static render: electric titles go static full-gradient (no flow/pulse/scan/flicker, no animated `drop-shadow`); `LogoMark` renders a static "lite" branch (one gradient-masked mark + a single static glow, skipping ~10 animated layers — via `useIsTouch()`); `GalaxyField` is **not mounted** (no WebGL context/rAF); `SiteBackground`'s `.site-bg-fx` mesh/cones freeze; and **all `backdrop-filter` blur is dropped** (backgrounds are already near-opaque). Desktop is untouched.
